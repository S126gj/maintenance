...
public class DictController {
    ...
    public R queryAll(@Parameter(description = "字典类型") @RequestParam(value = "type") String type,
                      @Parameter(description = "是否启用") @RequestParam(value = "enable") boolean enable,
                      Page page) {
        if (StringUtil.isBlank(type)) {
            return R.ok();
        }
        return R.ok().data(dictService.queryAll(EnumUtil.fromString(DictEnum.class, type), enable, page));
    }

    @Operation(summary = "新增字典")
    @PostMapping
    public R create(@Validated @RequestBody Dict dict){
